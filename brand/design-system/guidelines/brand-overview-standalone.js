...
(function () {
  ...
  async function buildFallback() {
    const map = { Button: window.__resources.btnJsx, Card: window.__resources.cardJsx, JobCard: window.__resources.jobcardJsx, Badge: window.__resources.badgeJsx, Tag: window.__resources.tagJsx, Input: window.__resources.inputJsx, Select: window.__resources.selectJsx };
    const files = ['Button', 'Card', 'JobCard', 'Badge', 'Tag', 'Input', 'Select'].map((n) => map[n]);
    let src = '';
  ...
  (async function () {
    ...
    root('cmp-cards', e(React.Fragment, null,
      e(Card, { image: window.__resources.imgZakelijk, interactive: true },
      ...
      e(JobCard, { title: 'Meewerkend chauffeur', image: window.__resources.imgTruck, category: 'Logistiek', location: 'Rotterdam', hours: 'Fulltime', teaser: 'Ben je graag onderweg en wil je dit combineren met je nieuwe baan?', readTime: '1,2 min' }),
    ));
